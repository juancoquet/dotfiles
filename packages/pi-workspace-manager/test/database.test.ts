...
function seed(registry: WorkspaceRegistry, sessionId = "session-1"): void {
  ...
  registry.upsertSession({
    id: sessionId, rootId: "root-1", sessionFile: `/sessions/${sessionId}.jsonl`, name: null,
    firstMessage: null, parentSessionFile: null, parentSessionId: null, lastActivityAt: null, archived: false, unread: true, sortRank: 1,
  });
}
...
test("creates private state automatically and persists manager metadata", () => {
  ...
  assert.deepEqual(reopened.getSession("session-1"), {
    id: "session-1", rootId: "root-1", sessionFile: "/sessions/session-1.jsonl", name: null,
    firstMessage: null, parentSessionFile: null, parentSessionId: null, lastActivityAt: null, archived: false, unread: true, sortRank: 1,
  });
